...
// Keep only the curated gallery-web cards in the public gallery.
(() => {
  const keepCuratedGalleryOnly = () => {
    document.querySelectorAll('.gallery-grid button').forEach((card) => {
      const image = card.querySelector('img');
      if (!image || !image.getAttribute('src')?.includes('/assets/gallery-web/')) card.remove();
    });
  };
  const galleryGrid = document.querySelector('.gallery-grid');
  if (!galleryGrid) return;
...
// Remove only the legacy fallback photo buttons wherever the old inline block inserts them.
(() => {
  const removeLegacyFallbackPhotos = () => {
    document.querySelectorAll('button').forEach((button) => {
      const label = button.getAttribute('aria-label') || '';
      const alt = button.querySelector('img')?.getAttribute('alt') || '';
      if (label === 'Open photograph' || label === 'Открыть дополнительное фото' || /Goa (excursion|group) — Dzen Retreat Goa/i.test(alt)) {
        button.remove();
      }
    });
  };
  const observer = new MutationObserver(removeLegacyFallbackPhotos);
...
<script>
(()=>{function compactGallery(){const g=document.querySelector('.gallery-grid');if(!g||g.dataset.compactReady==='1')return;const items=[...g.querySelectorAll('.gallery-item')];if(items.length<9)return;items.forEach((item,i)=>{if(i>=8)item.hidden=true});let b=document.querySelector('[data-gallery-more]');if(!b){b=document.createElement('button');b.type='button';b.dataset.galleryMore='1';b.className='gallery-more-button';b.innerHTML='<span data-gallery-more-ru>Посмотреть всю галерею</span><span data-gallery-more-en>View full gallery</span><b>↗</b>';g.parentElement.appendChild(b);b.addEventListener('click',()=>{const open=b.dataset.open==='1';items.forEach((item,i)=>{if(i>=8)item.hidden=!open});b.dataset.open=open?'0':'1';b.querySelector('[data-gallery-more-ru]').textContent=open?'Посмотреть всю галерею':'Свернуть галерею';b.querySelector('[data-gallery-more-en]').textContent=open?'View full gallery':'Collapse gallery'})}if(!document.getElementById('gallery-more-style')){const s=document.createElement('style');s.id='gallery-more-style';s.textContent='.gallery-more-button{display:inline-flex;align-items:center;gap:.8rem;margin:1.8rem auto 0;padding:.9rem 1.35rem;border:1px solid var(--ink);border-radius:999px;background:var(--paper);color:var(--ink);font:500 .72rem/1 DM Mono,monospace;letter-spacing:.04em;cursor:pointer;transition:all .2s}.gallery-more-button:hover{background:var(--ink);color:var(--paper);transform:translateY(-2px)}.gallery-more-button span+span{display:none}@media(min-width:801px){.gallery-more-button{margin-left:5vw}}';document.head.appendChild(s)}g.dataset.compactReady='1'}window.addEventListener('load',()=>{compactGallery();setTimeout(compactGallery,1200);setTimeout(compactGallery,3500)})})();
</script>



// Compact gallery: keep eight featured images visible and reveal the rest on demand.
(()=>{function compactGallery(){const g=document.querySelector('.gallery-grid');if(!g)return;const items=[...g.querySelectorAll('.gallery-item')];if(items.length<9)return;items.forEach((item,i)=>{if(i>=8)item.hidden=true});let b=g.parentElement.querySelector('[data-gallery-more]');if(!b){b=document.createElement('button');b.type='button';b.dataset.galleryMore='1';b.className='gallery-more-button';b.innerHTML='<span>Посмотреть всю галерею</span><b>↗</b>';g.parentElement.appendChild(b);b.addEventListener('click',()=>{const open=b.dataset.open==='1';items.forEach((item,i)=>{if(i>=8)item.hidden=!open});b.dataset.open=open?'0':'1';b.querySelector('span').textContent=open?'Посмотреть всю галерею':'Свернуть галерею'})}if(!document.getElementById('gallery-more-style')){const s=document.createElement('style');s.id='gallery-more-style';s.textContent='.gallery-more-button{display:inline-flex;align-items:center;gap:.8rem;margin:1.8rem auto 0;padding:.9rem 1.35rem;border:1px solid var(--ink);border-radius:999px;background:var(--paper);color:var(--ink);font:500 .72rem/1 DM Mono,monospace;letter-spacing:.04em;cursor:pointer;transition:all .2s}.gallery-more-button:hover{background:var(--ink);color:var(--paper);transform:translateY(-2px)}';document.head.appendChild(s)}}window.addEventListener('load',()=>{compactGallery();setTimeout(compactGallery,1200);setTimeout(compactGallery,3500)})})();
