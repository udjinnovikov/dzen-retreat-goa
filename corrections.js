  ...
  reviews.innerHTML = `
    ...
    <div class="reviews-layout">
      <article class="review-video-card">
        <video controls playsinline preload="metadata" poster="/assets/participants/participant-01-video.jpg?v=20260927-face-safe-1" src="/assets/review-01-web.m4v"></video>
...
(() => {
  ...
  const removeLegacyButtons = () => gallerySection.querySelectorAll('button:not(.gallery-item):not([data-gallery-more])').forEach((button) => button.remove());
...
// Gallery presentation only: keep eight approved photos visible and reveal the rest on demand.
(() => {
  const compactGallery = () => {
    const gallery = document.querySelector('.gallery-grid');
    if (!gallery || gallery.dataset.compactReady === '1') return;
    const items = [...gallery.querySelectorAll('.gallery-item')];
    if (items.length < 9) return;
    items.forEach((item, index) => { if (index >= 8) item.hidden = true; });
    let button = gallery.parentElement.querySelector('[data-gallery-more]');
    if (!button) {
      button = document.createElement('button');
      button.type = 'button';
      button.dataset.galleryMore = '1';
      button.className = 'gallery-more-button';
      button.innerHTML = '<span>Посмотреть всю галерею</span><b>↗</b>';
      ...
      button.addEventListener('click', () => {
        ...
        button.dataset.open = open ? '0' : '1';
        button.querySelector('span').textContent = open ? 'Посмотреть всю галерею' : 'Свернуть галерею';
      });
    }
    if (!document.getElementById('gallery-more-style')) {
      const style = document.createElement('style');
      style.id = 'gallery-more-style';
      style.textContent = '.gallery-more-button{display:inline-flex;align-items:center;gap:.8rem;margin:1.8rem auto 0;padding:.9rem 1.35rem;border:1px solid var(--ink);border-radius:999px;background:var(--paper);color:var(--ink);font:500 .72rem/1 DM Mono,monospace;letter-spacing:.04em;cursor:pointer;transition:all .2s}.gallery-more-button:hover{background:var(--ink);color:var(--paper);transform:translateY(-2px)}';
      document.head.appendChild(style);
    }
    gallery.dataset.compactReady = '1';
  };
  compactGallery();
  window.addEventListener('load', compactGallery);
  setTimeout(compactGallery, 1200);
  setTimeout(compactGallery, 3500);
})();
